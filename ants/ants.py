    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                assert (self.ant.is_container and not insect.is_container and not self.ant.contained_ant) or (insect.is_container and not self.ant.is_container), 'Two ants in {0}'.format(self)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        count = 0
        if self.max_range:
            while count <self.max_range and not p.bees:
                count+=1
                p = p.entrance
                if p.entrance==None:
                    return None
        elif self.min_range:
            while count> self.min_range and not p.bees:
                p = p.entrance
                if p.entrance == None:
                    return None
        else:
...
class ScubaThrower(ThrowerAnt):
    ...
    def __init__(self,name="Scuba Thrower",armor = 1):
        self.name = name
        self.armor = armor
        ...
        if self.is_TrueQueen:
            while p.exit != None:
                if p.exit.ant!=None:
                    ...
                    if p.exit.ant.is_container and p.exit.ant.contained_ant and p.exit.ant.contained_ant not in self.buffed:
                        p.exit.ant.contained_ant.damage*=2
                        self.buffed.append(p.exit.ant.contained_ant)
                    elif p.exit.ant not in self.buffed:
                        p.exit.ant.damage *= 2
                        self.buffed.append(p.exit.ant)
                p = p.exit
...
def make_slow(action, bee):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    def eoo_new_action(colony):
        if colony.time % 2 == 0:
            action(colony)
    return eoo_new_action
...
def make_scare(action, bee):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    def back_new_action(colony):
        bee.scared = True
        action(colony)
        bee.scared = False
    return back_new_action
    # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target not in self.scared_bee:
            self.scared_bee.append(target)
